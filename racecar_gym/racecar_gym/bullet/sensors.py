...
class Lidar(BulletSensor[NDArray[(Any,), np.float64]]):
    @dataclass
    class Config:
        accuracy: float
        rays: int
    ...
    def observe(self) -> NDArray[(Any,), np.float64]:
...
class RGBCamera(BulletSensor[NDArray[(Any, Any, 3), np.int32]]):
    @dataclass
    class Config:
        width: int
        height: int
    ...
    def observe(self) -> NDArray[(Any, Any, 3), np.int32]:
...
class AccelerationSensor(BulletSensor[NDArray[(6,), np.float64]]):
    @dataclass
    class Config:
        time_delta: float
    ...
    def observe(self) -> NDArray[(6,), np.float64]:
...
class VelocitySensor(BulletSensor[NDArray[(6,), np.float64]]):
    @dataclass
    class Config:
        gaussian_noise: float
...
class PoseSensor(BulletSensor[NDArray[(6,), np.float64]]):
    @dataclass
    class Config:
