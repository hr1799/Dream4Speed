...
class MultiEncoder(nj.Module):
  ...
  def __call__(self, data):
    ...
    data = {
        ...
        for k, v in data.items()} # Flattens out dim IDs specified in batch_dims
    outputs = []
    if self.cnn_shapes:
      inputs = jnp.concatenate([data[k] for k in self.cnn_shapes], -1)
      output = self._cnn(inputs)
      output = output.reshape((output.shape[0], -1)) # reshape to (batch_size, features)
      outputs.append(output)
    if self.mlp_shapes:
      inputs = [
          data[k][..., None] if len(self.shapes[k]) == 0 else data[k]
          for k in self.mlp_shapes] # read inputs to MLP from self.mlp_shapes dict
